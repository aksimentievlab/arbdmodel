...
class APBSRunner:
    ...
    def __init__(self,simconf=None, psize_script=None):
        """Initialize APBS interface.
        Args:
            psize_script: Optional path to psize script
            simconf: enviroment config
        """
        ...
        if simconf is not None:
            ...
            from sim_config import DefaultSimConf
            self.simconf=DefaultSimConf()
        
        self.psize = Path(psize_script) if psize_script else None
        
    def write_config(self, structure_name, xyz_dims, buffer=50, large_system='Off'):
        ...
        salt_conc=self.simconf.salt
        temperature=self.simconf.temperature
...
quit"""
    ...
    def run_calculation(self, structure_name, xyz_dims, work_dir=".", 
        ...
        try:
            ...
            tmp_file = self.work_dir / f"{structure_name}.elec.tmp.dx"
            out_file = self.work_dir / f"{structure_name}.elec.dx"
            Bound_grid(inFile=tmp_file, outFile=out_file,
